...
@ManagedBean
@ViewScoped
public class CtrlSeccionCincoPregunta2 {

    private CartesianChartModel lineModel;
    private PieChartModel pieModel;

    public CtrlSeccionCincoPregunta2() {
        this.init();
    }
    ...
    private CartesianChartModel lineChart() {
        ...
        try {
            ...
            for (int i = 0; i < ServiciosPostgrado.obtenerS5p2().size(); i++) {
                literalA.set(ServiciosPostgrado.obtenerS5p2().get(i).getS5p2(),
                        ServiciosPostgrado.obtenerS5p2DadoResp(ServiciosPostgrado.obtenerS5p2().get(i).getS5p2()).size());
            }
    ...
    private void createPieModel2() {
        ...
        try {
            for (int i = 0; i < ServiciosPostgrado.obtenerS5p2().size(); i++) {
                pieModel.set(ServiciosPostgrado.obtenerS5p2().get(i).getS5p2(),
                        ServiciosPostgrado.obtenerS5p2DadoResp(ServiciosPostgrado.obtenerS5p2().get(i).getS5p2()).size());
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
