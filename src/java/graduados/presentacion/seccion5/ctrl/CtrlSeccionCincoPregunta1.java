...
@ManagedBean
@ViewScoped
public class CtrlSeccionCincoPregunta1 {

    private CartesianChartModel lineModel;
    private PieChartModel pieModel;

    public CtrlSeccionCincoPregunta1() {
        this.init();
    }
    ...
    private CartesianChartModel lineChart() {
        ...
        try {
            ...
            for (int i = 0; i < ServiciosPostgrado.obtenerS5p1().size(); i++) {
                literalA.set(ServiciosPostgrado.obtenerS5p1().get(i).getS5p1(),
                        ServiciosPostgrado.obtenerS5p1DadoResp(ServiciosPostgrado.obtenerS5p1().get(i).getS5p1()).size());
            }
    ...
    private void createPieModel2() {
        ...
        try {
            for (int i = 0; i < ServiciosPostgrado.obtenerS5p1().size(); i++) {
                pieModel.set(ServiciosPostgrado.obtenerS5p1().get(i).getS5p1(),
                        ServiciosPostgrado.obtenerS5p1DadoResp(ServiciosPostgrado.obtenerS5p1().get(i).getS5p1()).size());
            }
        } catch (Exception e) {
            System.out.println(e.getMessage());
