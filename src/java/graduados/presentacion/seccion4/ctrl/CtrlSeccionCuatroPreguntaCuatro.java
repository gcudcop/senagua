...
@ManagedBean
@ViewScoped
public class CtrlSeccionCuatroPreguntaCuatro {

    private CartesianChartModel lineModel;
    private PieChartModel pieModel;

    public CtrlSeccionCuatroPreguntaCuatro() {
        this.init();
    }
        ...
        try {
            ...
            for (int i = 0; i < RespuestasSeccionCuatro.obtenerRespuestas().size(); i++) {
                literalA.set(RespuestasSeccionCuatro.obtenerRespuestas().get(i).getValoracion(),
                        ServiciosGradoSatisfaccion.obtenerS4p4dadoRespuesta(RespuestasSeccionCuatro.obtenerRespuestas().get(i).getCalificacion()).size());
            }
