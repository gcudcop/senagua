...
@ManagedBean
@ViewScoped
public class CtrlSeccionCuatroPreguntaDiez {

    private CartesianChartModel lineModel;
    private PieChartModel pieModel;

    public CtrlSeccionCuatroPreguntaDiez() {
        this.init();
    }
        ...
        try {
            ...
            for (int i = 0; i < RespuestasSeccionCuatro.obtenerRespuestas().size(); i++) {
                literalA.set(RespuestasSeccionCuatro.obtenerRespuestas().get(i).getValoracion(),
                        ServiciosGradoSatisfaccion.obtenerS4p10dadoRespuesta(RespuestasSeccionCuatro.obtenerRespuestas().get(i).getCalificacion()).size());
            }
