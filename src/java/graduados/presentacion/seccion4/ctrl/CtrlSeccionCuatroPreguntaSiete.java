...
@ManagedBean
@ViewScoped
public class CtrlSeccionCuatroPreguntaSiete {

    private CartesianChartModel lineModel;
    private PieChartModel pieModel;

    public CtrlSeccionCuatroPreguntaSiete() {
        this.init();
    }
        ...
        try {
            ...
            for (int i = 0; i < RespuestasSeccionCuatro.obtenerRespuestas().size(); i++) {
                literalA.set(RespuestasSeccionCuatro.obtenerRespuestas().get(i).getValoracion(),
                        ServiciosGradoSatisfaccion.obtenerS4p7dadoRespuesta(RespuestasSeccionCuatro.obtenerRespuestas().get(i).getCalificacion()).size());
            }
