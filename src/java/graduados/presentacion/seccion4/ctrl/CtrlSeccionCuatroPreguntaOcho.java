...
@ManagedBean
@ViewScoped
public class CtrlSeccionCuatroPreguntaOcho {

    private CartesianChartModel lineModel;
    private PieChartModel pieModel;

    public CtrlSeccionCuatroPreguntaOcho() {
        this.init();
    }
        ...
        try {
            ...
            for (int i = 0; i < RespuestasSeccionCuatro.obtenerRespuestas().size(); i++) {
                literalA.set(RespuestasSeccionCuatro.obtenerRespuestas().get(i).getValoracion(),
                        ServiciosGradoSatisfaccion.obtenerS4p8dadoRespuesta(RespuestasSeccionCuatro.obtenerRespuestas().get(i).getCalificacion()).size());
            }
